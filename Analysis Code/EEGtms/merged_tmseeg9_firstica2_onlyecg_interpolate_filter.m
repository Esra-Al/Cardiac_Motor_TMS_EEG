...
for s = 1:subnum
    ...
    %copy ica weights
    EEGica=pop_loadset([subfolder subid '_tmsall_default_1_2_3_4_5_6_7.set']);
    EEG.icasphere=EEGica.icasphere;
    EEG.icaweights=EEGica.icaweights;
    clear EEGica
    
    EEGcomps=pop_loadset([subfolder subid '_tmsall_default_1_2_3_4_5_6_7_8.set']);
    EEG.ecgrej=EEGcomps.ecgrej;
    clear EEGcomps
    
    EEG=eeg_checkset(EEG,'ica');
    EEG = pop_subcomp(EEG,EEG.ecgrej, 0);
    ...
    EEG = pop_saveset(EEG, 'filename', [subid '_tmsall_firstica2_onlyecg_interpolate_filter.set'], 'filepath', subfolder, 'version', '7.3');
end