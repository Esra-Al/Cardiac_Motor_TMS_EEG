...
for s = 1:subnum
    ...
    EEG = pop_loadset([subfolder subid '_tmsall_default_1_2.set']);
   
    EEG.nbchan_o=EEG.nbchan;
    EEG.trials_o=EEG.trials;
    EEG = pop_saveset(EEG, 'filename', [subid '_tmsall_default_1_2_3.set'], 'filepath', subfolder, 'version', '7.3');
    
    h1 = msgbox('Running ICA,now!');
    EEG = pop_runica(EEG, 'icatype' ,'fastica','g','tanh','approach','symm');
    EEG = eeg_checkset(EEG);
    close(h1);
    EEG = pop_saveset(EEG, 'filename', [subid '_tmsall_default_1_2_3_4.set'], 'filepath', subfolder, 'version', '7.3');
end