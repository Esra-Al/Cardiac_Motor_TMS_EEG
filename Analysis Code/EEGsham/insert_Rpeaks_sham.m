...
shamconds={'sham1', 'sham2', 'sham3', 'sham4'};
%%
...
clear EEG HEP
EEG=pop_loadset([eegpath subj_names{s} '_' shamconds{bl} '.set']);
%
ecg_chn = 32; %ECG electrode
ecg = double(EEG.data(ecg_chn,:));
ecg = ecg(:); % make sure ecg is one-column vect1or
...
%%
EEG = eeg_checkset(EEG,'eventconsistency');
pop_saveset(EEG, [eegpath subj_names{s} '_' shamconds{bl} '.set'])
